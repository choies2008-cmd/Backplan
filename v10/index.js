...
let state = JSON.parse(localStorage.getItem(KEY) || "null") || {
  ...
  settings: {
    ...
    calendarMonths: 12,
    categories: [{ id: "default", name: "기본", color: "#635bff" }],
    stampImage: "",
  },
  reschedules: 0,
};
state.settings.categories = state.settings.categories || [
  { id: "default", name: "기본", color: "#635bff" },
];
...
function taskHtml(t) {
  let done = t.done >= t.amount,
    c = category(t.g);
  return `<div class="task ${done ? "done" : ""}" onclick="toggleTask('${t.g.id}','${t.date}',${t.from})"><button class="check"></button><div class="txt"><b>${esc(t.g.title)}</b><div class="small"><span style="color:${c.color}">●</span> ${taskRange(t)}${esc(unitName(t.g))}</div></div></div>`;
}
...
function showPage(id) {
  ...
  if (id === "goals") {
    $("#allGoals").scrollIntoView({ block: "start" });
  }
}
function showAnnual() {
...
function previewPlan() {
  ...
  el.innerHTML = `가능한 계획일 <b>${p.length}일</b> · ${rs && re ? `${rs}~${re}${unitName(g)}` : `총 ${total}${unitName(g)}`} · 계획상 종료일 <b>${fmtDate(plannedEnd)}</b>${targetLater ? `<br>⚠ <b>주의:</b> 입력한 목표 날짜(${fmtDate(target)})보다 계획이 먼저 끝납니다. 그대로 진행하면 목표 날짜까지 억지로 늘리지 않고 <b>${fmtDate(plannedEnd)}</b>에 종료합니다.` : ""}`;
}
$("#prevMonth").onclick = () => {
  ...
  ${open ? `<div class="child-list">${ps.length ? ps.map((g) => goalRow(g)).join("") : '<div class="small">폴더가 비어 있습니다.</div>'}</div>` : ""}
    ...
    window.renderCalendar = function () {
      ...
      let selected = selectedGoals(),
        ...
        rows = [];
      // folders first according to calendar sort, then selected standalone plans not already in selected folders
      let items = combinedItems(state.settings.calendarSort),
        used = new Set();
      items.forEach((x) => {
        if (x.kind === "plan") {
          if (!selected.some((g) => g.id === x.g.id) || used.has(x.g.id))
            return;
          rows.push({ type: "plan", g: x.g });
          used.add(x.g.id);
        } else if (selectedFolderIds.has(x.id)) {
          let f = folderById(x.id);
          if (!f) return;
          rows.push({ type: "folder", f });
          let children = folderPlans(f).filter((g) =>
            selected.some((s) => s.id === g.id),
          );
          children.forEach((g) => used.add(g.id));
          ...
          if (from <= to) {
            ...
            let col =
              r.type === "curriculumLane"
                ...
                : category(r.g).color;
            html += `<div class="cell"><div class="plan-bar" style="left:calc(${left}% + 5px);right:calc(${100 - right}% + 5px);background:${alpha(col, 0.3)}" title="${esc(title)}"></div></div>`;
          } else html += '<div class="cell"></div>';
        });
      });
      grid.innerHTML = html;
